        ...
        <div class="row">
            ...
            <nav class="col-12 bg-light py-2">
                <ul class="nav justify-content-center">
                    <li class="nav-item">
                        <a href="{{ url('producto') }}" class="nav-link {{ Request::is('producto') ? 'active' : '' }}">Todos
                            los productos</a>
                    </li>
                    @foreach ($categorias as $categoria)
                        <li class="nav-item">
                            <a href="{{ url('producto/' . $categoria->id_categoria) }}"
                                class="nav-link {{ Request::is('producto/' . $categoria->id_categoria) ? 'active' : '' }}">{{ $categoria->nombre }}</a>
                        </li>
                    @endforeach
                </ul>
            </nav>
                        ...
                        <div class="col mb-4">
                            <div class="card border-0 shadow h-100">
                                ...
                                <div class="card-body">
                                    ...
                                    <p class="card-text text-danger"><strong>Descuento:
                                            Bs.{{ $producto->descuento }}</strong></p>
                                    <p class="card-text"><strong>Precio: Bs.{{ $producto->total_venta }}</strong></p>
                                    <a href="{{ route('vendedor.producto.show', $producto->id_producto) }}"
