    ...
    public function aplicarDescuentos()
{
    ...
    $promocionesActivas = Promocion::where('fecha_inicio', '<=', Carbon::now())
        ->where('fecha_fin', '>=', Carbon::now())
        ->get();

    foreach ($promocionesActivas as $promocion) {
        $producto = Producto::find($promocion->id_producto);

        if ($producto) {
            // Aplicar el descuento
            $descuento = $producto->precio * ($promocion->descuento / 100);
    ...
    return redirect()->route('admin.tablas.promocion.index')->with('success', 'Descuentos aplicados exitosamente.');
}
}
